...
class IRCClient:
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            # Server cert expired or invalid; retry once without verification so the bot stays up.
            if ssl_ctx is not None and ssl_ctx.verify_mode != ssl.CERT_NONE:
                log.warning("TLS verification failed (%s); reconnecting without certificate verification.", e)
                ssl_ctx = ssl.create_default_context()
                ssl_ctx.check_hostname = False
                ssl_ctx.verify_mode = ssl.CERT_NONE
                self.reader, self.writer = await asyncio.open_connection(server, port, ssl=ssl_ctx)
